    ...
    public class Halcon
    {
		...
		public static bool DetectionHalconRegion(int CamNum, int BaseNum, HWindow[] hWindow, HObject hImage, Parameters.DetectionSpec spec , HObject hObject ,ref List<DetectionResult>  detectionResult)
        {
            ...
            try
            {
				...
				HOperatorSet.SelectShape(ho_ConnectedRegions, out ho_SelectedRegions1, "area", "and", Parameters.detectionSpec[CamNum].AreaLow[BaseNum] / (Parameters.detectionSpec[CamNum].PixelResolutionRow * Parameters.detectionSpec[CamNum].PixelResolutionColum), spec.AreaHigh[BaseNum] / (Parameters.detectionSpec[MainForm.CamNum].PixelResolutionRow * Parameters.detectionSpec[CamNum].PixelResolutionColum));
			...
			else if (hvLength == 1)
			{
				...
				{
					HOperatorSet.CropPart(hImage, out detectionResult1.NGAreahObject, hv_Row - 500, hv_Column - 500, 1000, 1000);
					string stfFileNameOut = MainForm.productSheetNum + "-NG-" +"0";  // 默认的图像保存名称
				...
				{
					MessageBox.Show("存储路径有误，请确认");
					LogHelper.WriteWarn("存储路径有误，请确认");
				}
			...
			else
			{
                ...
                for (int i = 0; i < iAreaNum; i++)
				{
					...
					detectionResult1[i].ResultKind = (ImageErrorKind)BaseNum;
					...
					//HOperatorSet.SetColor(hWindow[0], "red");
					HalconColorSelect(BaseNum, hWindow[0]);
					HOperatorSet.SetTposition(hWindow[0], hv_Row12, hv_Column12);
					...
					HOperatorSet.SetTposition(hWindow[0], hv_Row12 + 500, hv_Column12);
					string Kind = HalconKind(BaseNum);
					HOperatorSet.WriteString(hWindow[0], "缺陷：" + Kind);
