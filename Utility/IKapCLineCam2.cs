...
namespace WY_App.Utility
{
    public class IKapCLineCam2
    {
        ...
        //
        // File name of image.
        public string m_strFileName = "D:/Image/Image2.tiff";

        public string m_strCamName = Parameters.commministion.productName + "/CamParams2.vlcf";
        // 当前帧索引。
        //
        // Current frame index.
        ...
        {
            LogHelper.WriteInfo("Start grabbing image2");
        }
        ...
        {
            LogHelper.WriteError("Grab frame lost2");
        }
        ...
        {
            LogHelper.WriteError("Grab image timeout2");
        }
        ...
        public void OnFrameReadyFunc(IntPtr pParam)
        {
            ...
            if (status.uFull == 1)
            {
                ...
                Halcon.hv_Width[2] = nFrameWidht;
                Halcon.hv_Height[2] = nFrameHeight;
                HOperatorSet.GenImage1(out MainForm.hImage[2], "byte", nFrameWidht, nFrameHeight, pUserBuffer);
        ...
        {
            LogHelper.WriteInfo("Stop grabbing image2");
        }
        #endregion
        ...
        private bool ConfigureCamera()
        {
            try
            {
                ...
                if (numCameras < 3)
                {
                    LogHelper.WriteInfo("No CL Cam2.");
                    IKapCLib.ItkManTerminate();
                    return false;
                }
                ...
                res = IKapCLib.ItkManGetDeviceInfo(2, ref di);
                ...
                if (di.DeviceClass == "CameraLink" && di.SerialNumber != "")
                {
                    ...
                    LogHelper.WriteInfo("Cam2: serial: {0}, name: {1}, interface: {2}.\n" + di.SerialNumber + di.FullName + di.DeviceClass);
                    IKapCLib.ITK_CL_DEV_INFO cl_board_info = new IKapCLib.ITK_CL_DEV_INFO();

                    // 打开相机。
                    //
                    // Open camera.
                    res = IKapCLib.ItkDevOpen(2, (int)ItkDeviceAccessMode.ITKDEV_VAL_ACCESS_MODE_EXCLUSIVE, ref m_hCamera);
                    CheckIKapC(res);

                    // 获取 CameraLink 相机设备信息。
                    //
                    // Get CameraLink camera device information.
                    res = IKapCLib.ItkManGetCLDeviceInfo(2, ref cl_board_info);
                    CheckIKapC(res);
        ...
        public bool ConfigureFrameGrabber()
        {
            try
            {
                ...
                m_strCamName = Parameters.commministion.productName + "/CamParams0.vlcf";
        ...
        #endregion
        public static bool StartGrabImage(IKapCLineCam2 grab)
        {
            try
            {
                int ret = 0;
        ...
        public static bool StopGrabImage(IKapCLineCam2 grab)
        {
            try
            {
                // 停止图像采集。
        ...
        public static bool ColseDevice(IKapCLineCam2 grab)
        {
            try
            {
                // 清除回调函数。
        ...
        public static bool OpenDevice(IKapCLineCam2 grab)
        {
            try
            {
                // 初始化 IKapC 运行环境。
