...
namespace WY_App.Utility
{
    public class IKapCLineCam0
    {
        ...
        //
        // File name of image.
        public string m_strFileName = "D:/Image/Image0.tiff";

        public string m_strCamName = Parameters.commministion.productName + "/CamParams0.vlcf";
        // 当前帧索引。
        //
        // Current frame index.
        ...
        {
            LogHelper.WriteInfo("Start grabbing image0");
        }
        ...
        {
            LogHelper.WriteError("Grab frame lost0");
        }
        ...
        {
            LogHelper.WriteError("Grab image timeout0");
        }
        ...
        public void OnFrameReadyFunc(IntPtr pParam)
        {
            ...
            if (status.uFull == 1)
            {
                ...
                Halcon.hv_Width[0] = nFrameWidht;
                Halcon.hv_Height[0] = nFrameHeight;
                HOperatorSet.GenImage1(out MainForm.hImage[0], "byte", nFrameWidht, nFrameHeight, pUserBuffer);
        ...
        private bool ConfigureCamera()
        {
            try
            {
                ...
                if (numCameras < 1)
                {
                    LogHelper.WriteInfo("No CL Cam0.");
                    IKapCLib.ItkManTerminate();
                    return false;
                }
                ...
                res = IKapCLib.ItkManGetDeviceInfo(0, ref di);
                ...
                if (di.DeviceClass == "CameraLink" && di.SerialNumber != "")
                {
                    ...
                    LogHelper.WriteInfo("Cam0: serial: {0}, name: {1}, interface: {2}.\n" + di.SerialNumber + di.FullName + di.DeviceClass);
                    IKapCLib.ITK_CL_DEV_INFO cl_board_info = new IKapCLib.ITK_CL_DEV_INFO();

                    // 打开相机。
                    //
                    // Open camera.
                    res = IKapCLib.ItkDevOpen(0, (int)ItkDeviceAccessMode.ITKDEV_VAL_ACCESS_MODE_EXCLUSIVE, ref m_hCamera);
                    CheckIKapC(res);

                    // 获取 CameraLink 相机设备信息。
                    //
                    // Get CameraLink camera device information.
                    res = IKapCLib.ItkManGetCLDeviceInfo(0, ref cl_board_info);
                    CheckIKapC(res);
        ...
        #endregion
        public static bool StartGrabImage(IKapCLineCam0 grab)
        {
            try
            {
                int ret = 0;
        ...
        public static bool StopGrabImage(IKapCLineCam0 grab)
        {
            try
            {
                // 停止图像采集。
        ...
        public static bool ColseDevice(IKapCLineCam0 grab)
        {
            try
            {
                // 清除回调函数。
        ...
        public static bool OpenDevice(IKapCLineCam0 grab)
        {
            try
            {
                // 初始化 IKapC 运行环境。
